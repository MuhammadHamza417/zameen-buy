...
export default function PropertyCard({ property, horizontal = false }) {
  return (
    <motion.div 
      ...
      className={`bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all group flex ${
    ...
    >
      <div className={`relative bg-gray-200 overflow-hidden shrink-0 ${
        ...
        />
        <button className="absolute top-3 right-3 p-2 bg-white/90 backdrop-blur-sm rounded-full text-gray-600 hover:text-red-600 shadow-md transition-all active:scale-90">
          <Heart size={18} />
        </button>
      </div>
